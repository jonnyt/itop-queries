###  Base VM Adds based on start_date  ###
Select
  "R" As `TYPE`,
  "" As `USER ID`,
  "W-VMBASE" As `ITEM ID`,
...
Where
  ...
  view_CustomerContract.end_date IS NULL AND
  view_VirtualMachine.id IS NOT NULL AND
  view_CustomerContract.start_date IS NOT NULL
Group By
	`SERVICEID DIRN`